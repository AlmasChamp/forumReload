...
func CanRegister(user entities.User) error {
	if !checkForSpace(user.Password1) || !checkForSpace(user.Name) {
	...
	if err := ValidatingPassword(user.Password1); err != nil {
		return err
	}
	fmt.Println("I CAN REGISTER")
...
func ValidatingPassword(password string) error {
	...
	var numbers, bigletters, smallletters bool
...
func ValidatingLogin(login string) error {
	...
	for _, v := range email[0] {
		if (v >= 32 && v <= 47) || (v < 32 || v > 122) || (v >= 58 && v <= 63) || (v >= 92 && v <= 96) {
			...
			return errors.New("Invalid email addres")
		}
	}
